...
Один job `checks` работает на GitHub-hosted Ubuntu 24.04 AMD64, не дольше 15 минут. Go выбирается из `toolchain` в `go.mod`; `GOTOOLCHAIN=local` запрещает дальнейшую автоматическую смену SDK. Actions закреплены commit SHA с версиями в комментариях; кеш setup-go отключён.
